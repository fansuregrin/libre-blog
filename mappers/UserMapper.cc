...
UserPtr UserMapper::selectByUsername(const std::string &username) {
...
UserPtr UserMapper::select(int id) {
    auto db = app().getDbClient();
    auto res = db->execSqlSync(
        "SELECT id,username,realname,email,password,create_time,modify_time,role "
        "FROM user WHERE id = ?",
...
int UserMapper::countByUsername(const std::string &username) {
...
int UserMapper::selectRoleId(int id) {
    auto db = app().getDbClient();
...
void UserMapper::insert(User &user) {
    auto db = app().getDbClient();
    auto res = db->execSqlSync(
...
int UserMapper::count() {
    auto db = app().getDbClient();
    return db->execSqlSync("SELECT COUNT(*) FROM user")[0][0].as<int>();
}

std::vector<User> UserMapper::selectLimit(int page, int pageSize) {
    auto db = app().getDbClient();
    auto res = db->execSqlSync(
...
void UserMapper::deletes(const std::vector<int> ids) {
    if (ids.empty()) return;
    auto db = app().getDbClient();
