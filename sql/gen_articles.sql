delimiter //
CREATE DEFINER=`root`@`%` PROCEDURE `genArticles`()
BEGIN
    ...
    while i <= 100 do
        INSERT INTO article (`title`,`author`,`category`,`content`,`create_time`) VALUES 
            (CONCAT('标题',i), FLOOR(RAND()*5)+1, FLOOR(RAND()*6)+1, CONCAT('我是内容',i), time);
        set i = i + 1;
